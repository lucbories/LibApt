...
LibaptFilter.create_from_string = function(arg_str, arg_model)
{
	var context = 'LibaptFilter.create_from_string(arg_str)';
	Libapt.trace_enter(context, '', LIBAPT_FILTER_TRACE);
	
	// SPLIT STRING
	var values = arg_str.split(',', 10);
	if (values.length < 5)
	{
		Libapt.trace_error(context, 'filter string is not valid with less than 5 items', true);
		return null;
	}
	...
	for(value_index in values)
	{
		...
		{
			attribute_name = 'group_mode';
		}
		
		if (attribute_name == 'join')
		{
			attribute_name = 'join_mode';
		}
		
		if (attribute_name == 'op')
		{
		...
		{
			attribute_value = arg_model.fields_set.get_field(attribute_value);
		}
		
		settings[attribute_name] = attribute_value;
	}
	
	// CREATE OBJECT
	var filter_obj = LibaptFilter.create(settings);
	
	Libapt.trace_leave(context, '', LIBAPT_FILTER_TRACE);
	return filter_obj;
}
...
LibaptFilter.create = function(arg_settings)
{
	...
	// TODO CHECK SETTINGS
	if ( Libapt.is_string(ext_settings.field) )
	{
		Libapt.trace_error(context, 'field name is not valid', true);
		return null;
	}
	
	// CREATE OBJECT
	var field_obj = new LibaptFilter(ext_settings.field, ext_settings.type, ext_settings.modifier, ext_settings.operator, ext_settings.var1, ext_settings.var2);
