...
class JSWRAPPER
{
	...
	static public $TRACE_JS_WRAPPER_MODELS		= array();
	
	/// @brief		Views to load during the page loading
	static public $TRACE_JS_WRAPPER_VIEWS		= array();
	...
	static public function addModelToLoad($arg_model)
	{
		...
		self::$TRACE_JS_WRAPPER_MODELS[] = $arg_model;
	}
	...
	static public function addViewToLoad($arg_view)
	{
		...
		self::$TRACE_JS_WRAPPER_VIEWS[] = $arg_view;
	}
	...
	static public function initResourcesToLoad($arg_create_resources = false)
	{
		...
		$js_code .= "			Libapt.use('apps-models-plugin');\n";
		if ($arg_create_resources)
		{
			foreach(self::$TRACE_JS_WRAPPER_MODELS as $model)
			{
		...
		// INIT VIEWS
		$js_code .= "			Libapt.use('apps-views-plugin');\n";
		$js_code .= "			Libapt.use('apps-views-factory');\n";
		$js_code .= "\n			var jqo_container = null;\n";
		$js_code .= "			var view = null;\n";
		$js_code .= "			var link = null;\n";
		foreach(self::$TRACE_JS_WRAPPER_VIEWS as $view)
		{
