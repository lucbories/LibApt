...
function libapt_storage_proto_tu()
{
	...
	trace_step(context, 'ENGINE TO STRING WITH DEFAULT VALUES', APT_MODELS_STORAGE_TRACE);
	var engine1 = new LibaptStorageRemoteJson('json_users');
	var engine1_str = engine1.to_string();
	// console.log(engine1_str);
	
	trace_separator(APT_MODELS_STORAGE_TRACE);
	trace_step(context, 'ENGINE TO STRING WITH CUSTOM VALUES', APT_MODELS_STORAGE_TRACE);
	...
	// trace_separator(APT_MODELS_STORAGE_TRACE);
	// trace_step(context, 'TEST READ SYNC', APT_MODELS_STORAGE_TRACE);
	// var datas1 = engine1.read_sync();
	// trace_var(context, 'datas1', datas1, APT_MODELS_STORAGE_TRACE);
	
	
	trace_separator(APT_MODELS_STORAGE_TRACE);
	trace_step(context, 'TEST READ ASYNC', APT_MODELS_STORAGE_TRACE);
	ajax_success_callback = function(datas, textStatus, jqXHR)
		{
			trace_step(context, 'ajax_success_callback', APT_MODELS_STORAGE_TRACE);
			trace_var(context, 'engine1.ajax_last_datas', engine1.ajax_last_datas, APT_MODELS_STORAGE_TRACE);
		};
	engine1.read(ajax_success_callback);
	
	
	// trace_separator(APT_MODELS_STORAGE_TRACE);
	// trace_step(context, 'TEST READ SYNC WITH FILTER', APT_MODELS_STORAGE_TRACE);
	...
	// trace_var(context, 'datas2', datas2, APT_MODELS_STORAGE_TRACE);
	
	
	// trace_separator(APT_MODELS_STORAGE_TRACE);
	// trace_step(context, 'TEST READ SYNC WITH ORDERS', APT_MODELS_STORAGE_TRACE);
	// var orders1 = '&model_orders="login=DESC"';
	// var datas3 = engine1.read_sync(orders1);
	// trace_var(context, 'datas3', datas3, APT_MODELS_STORAGE_TRACE);
	
	
	// trace_separator(APT_MODELS_STORAGE_TRACE);
	// trace_step(context, 'TEST INSERT ONE RECORD WITHOUT PASSWORD', APT_MODELS_STORAGE_TRACE);
	...
	// trace_separator(APT_MODELS_STORAGE_TRACE);
	// trace_step(context, 'TEST INSERT ONE RECORD WITH PASSWORD', APT_MODELS_STORAGE_TRACE);
	// var record1 = new Object();
	...
	// trace_separator(APT_MODELS_STORAGE_TRACE);
	// trace_step(context, 'TEST UPDATE ONE RECORD WITHOUT PASSWORD', APT_MODELS_STORAGE_TRACE);
	...
	// trace_separator(APT_MODELS_STORAGE_TRACE);
	// trace_step(context, 'TEST DELETE ONE RECORD', APT_MODELS_STORAGE_TRACE);
