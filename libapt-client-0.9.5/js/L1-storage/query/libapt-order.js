...
LibaptOrder.create_from_string = function(arg_str, arg_model)
{
	...
	Libapt.trace_enter(context, '', LIBAPT_ORDER_TRACE);
	
	// SPLIT STRING
	var parts = arg_str.split('=', 2);
	if (parts.length != 2)
	{
		Libapt.trace_error(context, 'order string is not valid', true);
		return null;
	}
	...
	if (! field)
	{
		Libapt.trace_error(context, 'bad field name [' + field_name + '] for model [' + arg_model.name + ']', true);
		return null;
	}
	...
	{
		Libapt.trace_error(context, 'bad order mode [' + mode_name + ']', true);
		return null;
	}
	...
	Libapt.trace_leave(context, '', LIBAPT_ORDER_TRACE);
	return order_obj;
}
...
LibaptOrder.create = function(arg_settings)
{
	...
	{
		Libapt.trace_error(context, 'order field is not valid', true);
		return null;
	}
	if ( Libapt.is_empty_str_or_null(ext_settings.mode) )
	{
		Libapt.trace_error(context, 'order mode is not valid', true);
		return null;
	}
