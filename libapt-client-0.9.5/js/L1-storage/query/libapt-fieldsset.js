...
function LibaptFieldsSet(arg_name, arg_fields)
{
	...
	// PUBLIC METHOD : GET A FIELD AT AN INDEX
	this.get_field_at = function(arg_index)
	{
		var context = 'get_field_at(' + arg_index + ')';
		this.enter(context, '');
		
		if (this.fields.length >= arg_index)
		{
			this.leave(context, 'not found');
			return null;
		}
...
LibaptFieldsSet.create = function(arg_settings)
{
	...
	{
		Libapt.trace_error(context, 'object name is not valid', true);
		return null;
	}
