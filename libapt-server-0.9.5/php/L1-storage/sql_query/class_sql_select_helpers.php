...
final class SQLSelectHelpers
{
	...
	static public function getSelectString($arg_fields, $arg_filters, $arg_orders, $arg_groups, $arg_slice_offset = null, $arg_slice_length = null, $arg_distinct = false, $arg_count = false, $arg_foreign_keys = false, $arg_joins = null, $arg_distinct_one_field = null)
	{
		...
		TRACE::enter($context, "", self::$TRACE_SELECT);
		...
		{
			return TRACE::leaveko($context, "no fields", "", self::$TRACE_SELECT);
		}
		...
		TRACE::step($context, "loop on fields", self::$TRACE_SELECT);
		foreach($arg_fields AS $key => $field)
		{
			$filter_field_name = $field->getName();
			TRACE::trace_var($context, "processed field", $filter_field_name, self::$TRACE_SELECT);
			...
			TRACE::trace_var($context, "processed is_distinct_one_field", $is_distinct_one_field, self::$TRACE_SELECT);
			TRACE::trace_var($context, "processed use_in_select", $use_in_select, self::$TRACE_SELECT);
...
//		TRACE::trace_var($context, "sql_distinct", $sql_distinct, self::$TRACE_SELECT);
		...
		if ($arg_count)
		{
			$query_string = "SELECT COUNT(*) AS count FROM ( $query_string ) AS tmp_count";
		}
		$query_string .= ";";
		
		TRACE::trace_var($context, "query_string", $query_string, self::$TRACE_SELECT);
		
		return TRACE::leaveok($context, "success", $query_string, self::$TRACE_SELECT);
	}
