...
function LibaptModel(arg_name, arg_source, arg_fields, arg_access, arg_options)
{
	...
	this.set_source = function(arg_value)
	{
		...
		{
			this.engine = (arg_value == 'local') ? arg_value : null;
		}
	}
...
LibaptModel.create = function(arg_settings)
{
	...
	trace_enter(context, '', LIBAPT_MODELS_MODEL_TRACE);
	...
	{
		trace_error(context, 'model source definition is not valid', LIBAPT_MODELS_MODEL_TRACE);
		return null;
	}
	if ( Libapt.is_empty_str_or_null(ext_settings.name) )
	{
		trace_error(context, 'model name definition is not valid', LIBAPT_MODELS_MODEL_TRACE);
		return null;
	}
	if ( ! Libapt.is_array(ext_settings.fields) || ext_settings.fields.length <= 0 )
	{
		trace_error(context, 'model fields definition is not valid', LIBAPT_MODELS_MODEL_TRACE);
		return null;
	}
	if ( typeof ext_settings.access != 'object' || ext_settings.access.length < 1 )
	{
		trace_error(context, 'model access definition is not valid', LIBAPT_MODELS_MODEL_TRACE);
		return null;
	}
	
	// CREATE FIELDS OBJECT
	var fields_obj = [];
	ext_settings.fields.forEach(
		function(field_item, index, array)
		{
			var field_obj = null;
			// trace_var(context, 'field_item', field_item, LIBAPT_MODELS_MODEL_TRACE);
			// trace_var(context, 'typeof field_item', typeof field_item, LIBAPT_MODELS_MODEL_TRACE);
			// trace_var(context, 'field_item instanceof LibaptField', field_item instanceof LibaptField, LIBAPT_MODELS_MODEL_TRACE);
			if (typeof field_item == 'object' && field_item instanceof LibaptField)
			{
				trace_step(context, 'already a field object', LIBAPT_MODELS_MODEL_TRACE);
				field_obj = field_item;
			}
			else if (typeof field_item == 'object' && ! (field_item instanceof LibaptField) )
			{
				trace_step(context, 'create field from settings', LIBAPT_MODELS_MODEL_TRACE);
				field_obj = LibaptField.create(field_item);
			}
			else
			{
				trace_error(context, 'field is not an object or a set of settings', LIBAPT_MODELS_MODEL_TRACE);
				return null;
			}
	...
	// trace_var(context, 'fields_obj', fields_obj, LIBAPT_MODELS_MODEL_TRACE);
	...
	trace_leave(context, '', LIBAPT_MODELS_MODEL_TRACE);
	return model_obj;
}
