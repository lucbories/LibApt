	/**
 * @file        libapt-mixin-query-editor-groups.js
 * @desc        Query groups editor
...
/**
 * @mixin				LibaptMixinQueryEditorGroups
 * @public
 * @desc				Mixin of query editor for groups
 */
var LibaptMixinQueryEditorGroups =
{
	/**
	 * @memberof			LibaptMixinQueryEditorGroups
	 * @public
	 ...
	 */
	draw_query_groups_editor: function(arg_model_view_obj, arg_container_jqo)
	{
		var self = this;
		var context = 'draw_groups_editor()';
		self.enter(context, '');
		...
		var label_jqo = $('<span>Edit the grouped fields of the query</span>');
		...
		for(field_key in arg_model_view_obj.query.fields_set.fields)
		{
			...
			if ( arg_model_view_obj.query.groups_set.has_group_for_field(field.name) )
			{
		...
		input_jqo.on("change", function()
			{
				...
				arg_model_view_obj.query.groups_set.remove_all_groups();
				...
				for(var index = 0 ; index < selected_fields_labels_array.length ; index++)
				{
					...
					{
						arg_model_view_obj.query.groups_set.add_group_for_field(field);
					}
				}
