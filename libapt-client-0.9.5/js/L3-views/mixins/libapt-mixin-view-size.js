...
var LibaptMixinViewSize =
{
	...
	{
		var self = this;
		self.push_trace(this.trace, this.mixin_view_size_trace);
		...
		// TEST IF THE VIEW IS IN A DIALOG WINDOW
		var view_is_in_dialog = self.content_jqo.parents().hasClass('ui-dialog');
		// console.log('view_is_in_dialog:' + view_is_in_dialog + ' for ' + self.name);
		...
		self.content_jqo.css('overflow-x', view_is_in_dialog ? 'none' : 'inherit');
		self.content_jqo.css('overflow-y', view_is_in_dialog ? 'none' : 'inherit');
		// self.container_jqo.css('overflow-x', view_is_in_dialog ? 'none' : 'inherit');
		// self.container_jqo.css('overflow-y', view_is_in_dialog ? 'none' : 'inherit');
		
		// GET TITLE BAR SIZE
		// if (self.container_jqo && self.titlebar_view && self.titlebar_view.content_jqo)
		// {
			// self.container_jqo.css('width', self.titlebar_view.content_jqo.width() + 'px');
		// }
		...
		if (content_width > container_width)
		{
			...
			{
				self.step(context, 'RULE 2: NOTHING TO DO IN DIALOG');
			}
		}
		...
		self.adjust_sizes_for_childs(view_is_in_dialog);
	 ...
	 * @return {boolean}	true:success,false:failure
	 */
	adjust_sizes_for_childs: function(arg_in_dialog)
	{
		var self = this;
		self.push_trace(this.trace, this.mixin_view_size_trace);
		...
		for(child_index in self.content_childs_jqo)
		{
			...
			if (child_width > container_width)
			{
				...
				if ( ! arg_in_dialog && child_jqo.width() > container_width )
				{
					self.content_jqo.css('overflow-x', 'scroll');
				}
			}
			...
			if (! arg_in_dialog && child_width < container_width)
			{
				self.step(context, 'child_width < container_width');
				child_jqo.css('width', container_width + 'px');
			}
			...
			if (! arg_in_dialog && child_height > container_height)
			{
