...
LibaptEditors.get_editor_jqo = function(arg_field_object, arg_record_values, arg_is_visible, arg_is_editable, arg_is_selector)
{
	...
	trace_enter(context, get_arg_not_null(arg_field_object.name, 'null field'), LibaptEditors.trace);
	
	// CHECK ARGUMENTS
	if ( Libapt.is_null(arg_field_object) )
	{
		trace_error(context, 'field is null', LibaptEditors.trace);
		return null;
	}
	...
	{
		trace_step(context, 'field is not visible', LibaptEditors.trace);
		...
		trace_leave(context, 'hidden field', LibaptEditors.trace);
		return $(jqo_str);
	}
	...
	{
		trace_step(context, 'field has a foreign link', LibaptEditors.trace);
		if ( ! arg_is_editable )
		{
			trace_step(context, 'field is not editable', LibaptEditors.trace);
			var value_str = get_arg_not_null(arg_record_values[arg_field_object.name], '');
			...
			trace_leave(context, 'visible not editable foreign field', LibaptEditors.trace);
			return jqo;
		}
		...
		trace_leave(context, 'visible editable foreign field', LibaptEditors.trace);
		return jqo;
	}
	...
	{
		trace_step(context, 'field is part of join', LibaptEditors.trace);
		...
		if ( ! arg_is_editable )
		{
			...
			trace_leave(context, 'visible not editable join field', LibaptEditors.trace);
			return jqo;
		}
		...
		trace_leave(context, 'visible editable join field', LibaptEditors.trace);
		return jqo;
	}
	...
	if ( ! arg_is_editable )
	{
		...
		trace_leave(context, 'visible not editable field', LibaptEditors.trace);
		return $(jqo_str);
	}
	...
	if (arg_is_selector)
	{
		trace_step(context, 'field editor should be a selector', LibaptEditors.trace);
		...
		trace_leave(context, 'visible editable field selector', LibaptEditors.trace);
		return jqo;
	}
	...
	trace_leave(context, 'success', LibaptEditors.trace);
	return jqo;
}
 ...
 * @return {object}		jquery object
 */
LibaptEditors.get_select_editor_jqo = function(arg_field_object, arg_record_values, arg_key_field_name, arg_values_fields_names, arg_values_format, arg_values_separator, arg_model_view, arg_all_label, arg_all_value)
{
	...
	trace_enter(context, '', LibaptEditors.trace);
	...
	{
		trace_error(context, 'key field name is empty', LibaptEditors.trace);
		return null;
	}
	if ( ! Libapt.is_array(arg_values_fields_names) )
	{
		trace_error(context, 'values fields names is not an array', LibaptEditors.trace);
		return null;
	}
	...
	// DEBUG
	trace_var(context, 'arg_model_view', Libapt.is_null(arg_model_view) ? 'null' : arg_model_view.name, LibaptEditors.trace);
	// trace_var(context, 'arg_record_values', arg_record_values, LibaptEditors.trace);
	// trace_var(context, 'arg_key_field_name', arg_key_field_name, LibaptEditors.trace);
	// trace_var(context, 'arg_values_fields_names.length', arg_values_fields_names.length, LibaptEditors.trace);
	// trace_var(context, 'arg_values_fields_names', arg_values_fields_names, LibaptEditors.trace);
	// trace_var(context, 'arg_values_format', arg_values_format, LibaptEditors.trace);
	// trace_var(context, 'arg_values_separator', arg_values_separator, LibaptEditors.trace);
	...
	jqo_refresh_div.click(
		function()
		{
			trace_enter(context + '.refresh_button.click', '', LibaptEditors.trace);
			...
			{
				trace_step(context + '.refresh_button.click', 'arg_model_view is not null and is instance of LibaptModelView', LibaptEditors.trace);
				var groups = null;
			...
			{
				trace_step(context + '.refresh_button.click', 'arg_model_view is null or not instance of arg_model_view', LibaptEditors.trace);
			...
			for(record_index in records)
			{
				...
				// trace_var(context, 'record', record, LibaptEditors.trace);
				// trace_var(context, 'selected', selected, LibaptEditors.trace);
				
				// GET RECORD VALUE
				var key_value	= record[arg_key_field_name];
				// trace_var(context, 'key_value', key_value, LibaptEditors.trace);
				...
				// trace_var(context, 'record_values', record_values, LibaptEditors.trace);
				...
				if ( ! selected && arg_values_format == 'concat')
				{
					for(record_value_key in record)
					{
						...
						trace_var(context, 'selected? concat loop record value', current_record_value, LibaptEditors.trace);
						
						var value = Libapt.is_object(selected_value) ? selected_value[record_value_key] : selected_value;
						
						if (current_record_value == value)
						{
							trace_step(context, 'current value is selected', LibaptEditors.trace);
							selected = true;
						}
					}
				}
			...
			{
				// jqo_select.children().removeAttr('selected');
				// jqo_select.prepend( $('<option value="' + arg_all_value + '" selected="selected">' + arg_all_label + '</option>' ) );
				jqo_select.prepend( $('<option value="' + arg_all_value + '">' + arg_all_label + '</option>' ) );
			}
			
			trace_leave(context + '.refresh_button.click', '', LibaptEditors.trace);
			return this;
		}
	);
...
LibaptEditors.get_field_values = function(arg_field_object, arg_record_values, arg_key_field_name, arg_values_fields_names)
{
	...
	for(value_field_name_key in arg_values_fields_names)
	{
		...
		trace_var(context, 'value_field_name', value_field_name, LibaptEditors.trace);
		...
		trace_var(context, 'current_record_value', current_record_value, LibaptEditors.trace);
...
LibaptEditors.get_field_values_str = function(arg_field_object, arg_record_values, arg_selected_value, arg_values_format, arg_values_separator)
{
	...
	// DEBUG
	// trace_var(context, 'arg_selected_value', arg_selected_value, LibaptEditors.trace);
	// trace_var(context, 'arg_record_values', arg_record_values, LibaptEditors.trace);
