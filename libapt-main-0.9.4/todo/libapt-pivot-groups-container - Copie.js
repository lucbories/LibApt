...
function LibaptPivotGroupsContainer(arg_name)
{
	...
	this.get_group = function(arg_group, arg_label)
	{
		...
		{
			field_name = arg_group.name;
			field = arg_group;
			var group_label = arg_label ? arg_label : arg_group.label;
			group = new LibaptPivotGroup(field_name, field, group_label);
		}
		
		// GIVEN GROUP IS A FIELD NAME
		else if ( Libapt.is_string(arg_group) )
		{
			field_name = arg_group;
			field = this.query.get_field(field_name);
	...
	// GRID METHOD : ADD A COLUMN GROUP
	this.add_col_group = function(arg_group, arg_label)
	{
		var context = 'add_col_group(group[,label])';
		this.enter(context, '');
		
		var group = this.get_group(arg_group, arg_label);
		this.col_groups[field_name] = group;
		this.col_groups_array.push(group);
