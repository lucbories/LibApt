...
abstract class AbstractSQLModelImpl extends AbstractSQLModel
{
	...
	public function readCount($arg_fields, $arg_filters, $arg_orders, $arg_options)
	{
		...
		// $fields = $this->fields_set->getFields();
		$query = $this->createQuery(AbstractQuery::$TYPE_SELECT_COUNT, $arg_fields);
		$query->setFilters($arg_filters);
		$query->setOrders($arg_orders);
		$query->setOptions($arg_options);
		...
		$count_record = $datas_records[0];
		
		return TRACE::leaveok($context, "datas records are OK", $count_record, self::$TRACE_MODEL);
	}
	
	public function readCountWithSlice($arg_fields, $arg_filters, $arg_orders, $arg_slice_offset, $arg_slice_length, $arg_options)
	{
		$context = get_class($this).".AbstractSQLModelImpl.readCountWithSlice()[".$this->getName()."]";
