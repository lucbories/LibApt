...
class SQLFiltersHelpers
{
	...
	*/
	...
	static public function getOperatorSQLString($arg_operator, $arg_type, $arg_column, $arg_var1, $arg_var2)
	{
		...
		// ALL TYPES OPERATORS
		if ($arg_operator == "isnull")
		{
			return " ISNULL($arg_column)";
		}
		if ($arg_operator == "isnull")
		{
			return " NOT ISNULL($arg_column)";
		}
		...
		if ( (! is_null($arg_var1) ) && $arg_type == "String")
		{
			...
			elseif ($arg_operator == "notequals" || $arg_operator == "!=")
			{
				return $arg_column." <> '".$arg_var1."'";
			}
			...
			{
				// SPLIT THE STRING INTO AN ARRAY WITH ':' SEPARATOR
				$str_array = explode(":", $arg_var1);
				
				// CHECK IF IT IS AN ARRAY OR A SINGLE VALUE
				if ( ! is_array($str_array) && ! is_null($arg_var1) && $arg_var1 != "")
				{
					$str_array = array($arg_var1);
				}
				
				// REPLACE SEPARATOR ESCAPE CHARACTER
				foreach($str_array as $key=>$str_value)
				{
					$str_array[$key] = str_replace("\doublepoint", ":", $str_value);
				}
				
				// MERGE THE ARRAY STRINGS WITH THE SQL SEPARATOR ','
